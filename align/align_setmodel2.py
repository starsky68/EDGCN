...
class load_data(object):
    def __init__(self, config, model_type='E'):
        ...
        with open(config.datasetPath + 'pre/kgs_num', 'r') as ff:
            datasetname = config.datasetPath.split('/')[1]
            if "15K" in datasetname:
                ...
                #print(ff.readline()[:-1].split(':')[0],'11111111111111111111111111')
                self.kg_V = int(ff.readline()[:-1].split(':')[1])
                #print(ff.readline()[:-1].split(':'),'11111111111111111111111111')
        ...
        if model_type == 'M':
            ...
            KGs_attr_triple = KGs_attr_triple.astype(np.int64)
...
class modelClass2():
    ...
    def model_train(self, epochs_beg=0):
        ...
        self.compute_test(epochs_i, 'epoch_best')
