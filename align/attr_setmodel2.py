...
from autil.mydataset import MyDataset
...
class modelClass2():
    ...
    def model_train(self, epochs_beg=0):
        ...
        for epochs_i in range(epochs_beg, self.config.train_epochs):  # epochs=1001
            ...
            # loss、acc
            # self.mymodel, self.optimizer = amp.initialize(self.mymodel, self.optimizer, opt_level="O1")
            loss_train, e_out_embed = self.mymodel.get_loss(es_embed, ec_embed, self.train_neg_pairs_es, self.train_neg_pairs_ec)  # 求解loss:
            ...
            if epochs_i >= self.config.start_valid and epochs_i % self.config.eval_freq == 0:
                ...
                loss_val, e_out_embed = self.mymodel.get_loss(es_embed, ec_embed, self.valid_neg_pairs_es, self.valid_neg_pairs_ec)
