...
class HET_align2(nn.Module):  # herited classes: nn.Module
    def __init__(self, input_data, config):
        ...
        self.fc = nn.Linear(30000, 300)
        self.special_spmm = SpecialSpmm()  # sparse matrix multiplication
        # self.special_spmm = torch.sparse.mm()
    ...
    #
    def XNorm(self, x, gamma=0.0001):
        norm_tensor = torch.norm(x, p=2, keepdim=True)
    ...
    def forward(self):
        ...
        # e_embed_2 = e_embed_2.unsqueeze(0)
        # e_embed_2 = e_embed_2.view(e_embed_2.size(0), e_embed_2.size(1), -1)
        print('e_embed_2', e_embed_2.size())
        # Equation 12~14经过告诉公路网络的实体
        e_embed_2 = self._bigru(e_embed_2)
        print('e_embed_2_gru', e_embed_2.size())
        gcn_e_1 = self.add_diag_layer(e_embed_2)  # (E,dim)
        print('gcn_e_1', gcn_e_1.size())
        gcn_e_2 = self.highway(e_embed_2, gcn_e_1)
        print('gcn_e_2', gcn_e_2.size())
        gcn_e_3 = self.add_diag_layer(gcn_e_2)
        output_layer = self.highway(gcn_e_2, gcn_e_3)  # (E,dim)
        print('output_layer', output_layer.size())
        # 改进
        ######################################dddd
        output_layer = output_layer + 1e-4 * self.dist(gcn_e_2, output_layer)
        print('output_layer_end', output_layer.size())
        return output_layer

    # 双向GRU网络
    def _bigru(self, e_layer1):
        e_layer1 = e_layer1.view(-1)
        print('e_layer1', e_layer1.size())
        gru, _ = torch.nn.GRU(e_layer1, hidden_size=50, num_layers=2, batch_first=True, bidirectional=True)
        print('gru', gru.size())
        out = self.fc(gru)
        return out
    # add a highway layer
    def highway(self, e_layer1, e_layer2):
        # (E,dim) * (dim,dim)
        print('e_layer11111', e_layer1.size())
        transform_gate = torch.mm(e_layer1, self.highwayWr) + self.highwaybr.squeeze(1)
        print('transform_gate11111', transform_gate.size())
    ...
    def add_e_att_layer(self, e_inlayer, r_layer):
        ...
        ee_atten = torch.exp(
            -self.leakyrelu(torch.matmul(eer_embed, self.atten_r).squeeze()))  # (D,r_dim)*(r_dim,1) => D
    ...
    def add_r_layer(self, e_inlayer):
        ...
        # r_embed = self.XNorm(r_embed, gamma=0.00001)
